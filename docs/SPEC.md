...
| バージョン | v1.2 |
| 前版 | v1.1（意思決定の記録） |
| 状態 | **未検証のまま M1 の実装に進むと決定**（§4）。R-01 は未緩和のまま残る |
...
### v1.1 → v1.2 の変更（意思決定者の判断による）

| # | 変更 | 判断 |
| --- | --- | --- |
| 1 | **M0（2週間の事前検証）を実施しない**（§4 を全面書き換え） | 「数字が悪くても作る」と決定。撤退しないなら検証は判断材料を持たないため、時間を実装に回す |
| 2 | **ゲーティングを撤廃**。フィードは誰でも見られる（D-01 を反転） | 閲覧を止めるのは「追い立てる」体験であり、コンセプトに反すると判断 |
| 3 | 代わりに **リアクションを参加者限定**にした（D-01） | 圧をかけずに参加動機を残すための代替設計 |
| 4 | **K-2（参加率）の目標を 45% → 30% に下方修正**（§12） | ゲーティングという最強の強制力を外した以上、同じ数字は出ない |
| 5 | **技術スタックを確定**（§10.3、U-05 を解決） | 実装を Claude Code が書くと決定したため、それに最適な構成を選定 |
| 6 | **R-10（閲覧専用ユーザーが多数派になる）を新設**、R-01・R-02 を更新（§15） | ゲーティング撤廃と検証省略の直接の帰結 |
| 7 | 事業化の意思を確定（§13） | 「ゆくゆくは事業」と決定。収益セクションを維持 |
...
| 2 | ~~ゲーティング（投稿しないと今日のフィードを見られない）を明文化~~（D-01） → **v1.2 で撤廃** | 参加率を決める最重要ロジックが未定義だった。v1.2 で「閲覧は自由・リアクションは参加者のみ」に反転 |
...
| --- | --- | --- | --- |
| **BeReal** | 通知から2分・加工なし・**投稿しないと見られない** | 急成長 → 飽きで失速 → 買収 | ゲーティングは参加率に効くが、ODAI は思想的理由で採用しない（D-01）。一方、**「毎日同じ体験」は飽きる**（→ D-04 の可変リアクション、§7 のお題運用で毎日の変化を作る） |
| **Poparazzi** | いいね廃止・自撮り禁止・フォロワー概念の再設計 | DL1位 → 失速 → クローズ | **承認欲求を消すと、開く理由も一緒に消える**。ODAIの答えは「お題」だが、**これは未検証のまま実装に進む最大の賭け**（§4 / R-01） |
...
### 決定：事前検証（M0）は実施せず、実装に進む

**決定**

v1.1 が定めていた「実装前に2週間の検証を行い、H-1 が 20% を切ったら実装に進まない」という
ゲートを**外す**。M0 を実施せず、M1（実装）に直接入る。

**理由**

意思決定者の判断は「**数字が悪くても作る**」。
検証の唯一の価値は**撤退の判断材料**にあるので、撤退しないと決まっているなら、
M0 は2週間を消費するだけで何も決めない。**やるフリが一番コストが高い。**

**この決定で引き受けるリスク（明記して進む）**

| | |
| --- | --- |
| 未緩和のまま残るリスク | **R-01：お題では毎日開く理由にならない**（Poparazzi 型の失速） |
| 影響 | 致命的。これが外れていると、他のすべての設計が意味を失う |
| いつ判明するか | **M1（クローズドβ）が事実上の最初の検証になる**。判明が数ヶ月遅れ、その時点で実装コストは既に支払い済み |

**したがって、M1 の設計を変える**

M1 は「作って出す」だけでなく、**H-1 相当を必ず測る**フェーズとして設計する。

- KPI 計測（§12）を**後付けではなく最初から**実装する。特に K-2（参加率）と K-5（お題別参加率）
- β開始から14日目に、`初日投稿者のうち14日目も投稿した割合`を必ず算出する
- ここが 20% を大きく下回った場合、**M2（一般公開）に進む前に**設計を見直す
  （撤退はしないが、**同じ設計のまま広げない**）
...
| ID | 仮説 | M1 での測り方 |
| --- | --- | --- |
| **H-1** | お題があれば、人は毎日投稿する | β開始14日目の継続率（初日投稿者ベース） |
| **H-2** | 他人の回答を見るのが面白い | フィード閲覧率・スクロール深度（§12 `feed_scroll_depth`）+ ヒアリング n=10 |
| **H-3** | お題の良し悪しで参加率が大きく変わる | K-5（お題別参加率）の最大 ÷ 最小 |

### M0 キットの扱い

[`docs/m0/`](m0/README.md) は破棄せず、**用途を変えて残す**。

| ファイル | 転用先 |
| --- | --- |
| [`m0/prompts.md`](m0/prompts.md) | お題の初期ストックと、各お題の設計意図の記録。§7 の運用にそのまま使える |
| [`m0/interview.md`](m0/interview.md) | M1 β参加者へのヒアリング台本（H-2 の測定に必要） |
| [`m0/report.py`](m0/report.py) | β初期、計測基盤が整うまでの手動集計に使える |
| [`m0/attendance.csv`](m0/attendance.csv) / [`m0/daily-log.csv`](m0/daily-log.csv) | 同上 |

> **もし気が変わって M0 をやりたくなったら、キットはそのまま動く。** 3日で始められる。

---

## 5. 主要な設計決定

### D-01. フィードは誰でも見られる。ただしリアクションは参加者だけ

**決定**

- **今日のフィードも、過去のアルバムも、投稿していなくても自由に閲覧できる**
- ただし **リアクションを押せるのは、その日のお題に参加した人だけ**
- 未参加でフィードを見ているとき、上部に柔らかいカードを1枚だけ出す
...
- カードは**閉じられる**。閉じたらその日はもう出さない
- 未参加を責める表現（「あなただけ参加していません」等）は使わない

**理由**

v1.1 では BeReal 型のゲーティング（投稿するまでフィードをブラー）を採用していたが、
**閲覧を人質に取るのは「追い立てる」体験**であり、
「SNS疲れしない」というこのプロダクトの中心と正面から矛盾する。ここは思想を優先する。

一方で参加の動機をゼロにはできないので、**見るのは自由 / 関わるには参加**という非対称にした。
リアクションは「他人の投稿に触れる」行為なので、参加者に限るのは筋が通っている。

**この決定のトレードオフ（受け入れる）**

| 失うもの | 得るもの |
| --- | --- |
| **参加率は確実に下がる**（K-2 の目標を 45% → 30% に修正、§12） | **追い立てないSNSであるという一貫性** |
| 閲覧専用ユーザーが増え、フィードが痩せるリスクが上がる（R-02 / **R-10**） | **Web で「今日のお題」ページを公開できる**。ログイン不要で中身が見えるので、そのまま新規獲得の導線と検索流入の資産になる（ゲーティングがあると不可能） |

> 2つ目は事業（§13）にとって想像以上に大きい。
> ゲーティングは参加率と引き換えに、**プロダクトを外から一切見えなくする**。
> 閲覧を開いたことで、獲得コストを下げる打ち手が新しく手に入った。

**採用しなかった案**

- **ゲーティング（v1.1 の案）**：参加率には最も効くが、思想と矛盾する
- **過去だけ開いて今日は閉じる**：v1.1 の折衷案。同じ理由で不採用
- **未参加でも全部できる（リアクションも可）**：参加の動機が完全に消える。ストリークと通知だけでは弱い（R-09）
...
- 1つのお題につき投稿は**1人1枚**
- 投稿の**差し替えは不可**、**削除は可能**（削除すると、その日は未参加に戻り、リアクションも押せなくなる）
...
flowchart TD
    ...
    B --> C[プレビュー + ひとこと]
    C -->|投稿| D[今日のフィード]
    A -->|未投稿でも見られる| E[今日のフィード<br/>リアクションは押せない]
    E -->|参加する| B
...
- 下にスクロールすると、**今日のフィードがそのまま見られる**（D-01）。
  リアクションは押せず、上部に「まだ参加していません」のカードが1枚出る（閉じられる）
...
- タップで拡大表示、そこで**その日専用のリアクション**（D-04）を押せる
  — ただし**その日のお題に参加している場合のみ**（D-01）。未参加時はリアクションが淡色で表示され、
  タップすると参加を促すカードが出る
...
| `GET` | `/v1/feed?odai_id=&sort=new\|top&scope=country\|world&cursor=` | フィード。**閲覧は誰でも可**。未参加の場合は `can_react: false` を返す（D-01） |
| `PUT` | `/v1/posts/{id}/reaction` | リアクション設定/変更。**その日のお題に未参加なら 403** |
...
### 10.3 技術スタック（確定 / U-05 解決済み）

**前提：実装は Claude Code が書く。** したがって選定基準を「人間のチームにとっての最適」ではなく、
**「AI が一貫して書き切れて、型で守られ、枯れたパターンが多い構成」**に置く。

| 層 | 確定 | 理由 |
| --- | --- | --- |
| モバイル | **Expo（React Native）+ TypeScript** | 1コードベースで iOS/Android。`expo-camera` でD-02の要件が素直に書ける。**EAS Build でストア提出まで完結**し、Xcode の手作業を最小化できる |
| 言語 | **TypeScript（フロント・バックとも）** | 全レイヤが1言語。型定義をAPIとアプリで共有でき、AIが書くときの取り違えが激減する |
| バックエンド | **Supabase**（Postgres + Auth + RLS + Storage） | 認証・DB・ストレージ・関数が1つに揃う。**ブロック（§8）とリアクション権限（D-01）をRLSで宣言的に書ける**のが決め手 |
| 画像 | **端末側でリサイズ + Exif除去 → Supabase Storage** | 端末で 1440px/WebP に落としてから送る（§11.1）ので転送も原価も最小。**画像URLの生成は1モジュールに閉じ、R2 へ移行できる形にしておく**（§11.2） |
| 自動判定 | **Supabase Edge Function → Cloud Vision SafeSearch** | §8.1 の必須要件1。投稿完了の裏で非同期に走らせ、投稿体験を止めない |
| プッシュ | **Expo Notifications**（FCM/APNs をラップ） | §9 は1日最大2回なので高機能な配信基盤は不要 |
| 分析 | **自前イベント → Postgres**（§12.2 の定義そのまま） | 外部SaaSを入れる前に、まず自分のDBで K-2 / K-5 が見られる状態を作る |
| Web公開ページ | **Next.js（M2 以降）** | D-01 で閲覧を開いたことで可能になった獲得導線。「今日のお題」をログイン不要で見せる |

**この構成で避けているもの**

- Flutter（Dart）：アプリは良いが、バックエンドと言語が分かれ、共有できる型が無くなる
- 自前のサーバ実装（Node + ORM + 認証を自作）：認証とストレージの自作は、AI に書かせるときバグの温床になりやすい
- 初手からの Cloudflare R2：正しい選択だが、Supabase Storage の方が最初の1行までが速い。**移行できる形にしておけば十分**（§11.2 の試算が効いてくるのは DAU が万を超えてから）
...
| **K-2** | **参加率** | その日のアクティブユーザーのうち投稿した割合 | **≥ 30%** |
...
| **K-8** | **閲覧専用率** | アプリを開いたが14日間一度も投稿しなかったユーザーの割合 | **< 40%**（超えたら D-01 の再考） |

> **K-2 と K-5 がこのプロダクトの心臓**。日次で見る。

**K-2 の目標値について**：v1.1 では 45% だったが、D-01 でゲーティング（＝参加率に最も効く仕組み）を
外したため **30% に下方修正**した。BeReal 型の強制力なしでこの水準を維持できるかが、
このプロダクト最大の未検証点であり、**K-8 はその副作用を監視するために新設した指標**である。
...
**前提**：事業化の意思は確定している（「ゆくゆくは事業」）。したがって、
このセクションは「いつかやるかもしれない構想」ではなく、**M2 で着手する計画**として扱う。
...
| --- | --- | --- | --- |
| ~~M0 検証~~ | — | **実施しない**（§4 の決定） | — |
| **M1 クローズドβ** | 6〜8週 | iOS + Android。1大学/1コミュニティ 300人。カメラ・フィード・アルバム・通報・計測 | K-1 ≥ 60% / **K-2 ≥ 30%** / 14日継続率を必ず算出（§4） |
| **M2 国内公開** | +8週 | Web公開ページ（D-01）、モデレーション体制、スポンサードお題の営業開始 | K-3 ≥ 35% / K-8 < 40% / 通報対応24h体制が回る |
...
| リアクションの参加者限定（D-01） | 世界マップ |
| お題ごとのリアクション（D-04） | AIお題生成 |
| 通報・ブロック・自動判定（§8） | プレミアム / 課金 |
| ストリーク（フリーズ込み） | ウィジェット |
| **KPI計測（§12.2 のイベント）** ← 後付けにしない | Web公開ページ（M2へ） |

**実装順序**（依存関係の順。各段階で動くものが残る形にする）

1. 認証 + プロフィール（Supabase Auth、生年月日による16歳判定）
2. お題マスタ + ホーム画面（今日のお題・残り時間、D-03 のタイムゾーン処理）
3. カメラ + 投稿（D-02 のその場撮影のみ、端末側リサイズとExif除去）
4. フィード（誰でも閲覧、D-01）
5. リアクション（参加者限定。RLSで担保）
6. 通報・ブロック・自動判定（**§8。ストア提出の前提条件なのでM1内で必ず終わらせる**）
7. ストリーク + 通知（D-06 / §9）
8. アルバム（D-09）
9. KPI計測イベント（§12.2）

> **6 を後回しにしない。** ここが無いとストアに出せず、M1 の完了条件を満たせない。
...
| ID | リスク | 影響 | 確率 | 対策 |
| --- | --- | --- | --- | --- |
| **R-01** | **お題では毎日開く理由にならない**（Poparazzi 型の失速） | 致命 | 中 | **未緩和。** M0 を実施しない決定（§4）により、M1 のβまで判明しない。せめて M1 で14日継続率を必ず測り、**M2 に進む前に**設計を見直せるようにする |
| **R-02** | 初期の投稿数が少なく、フィードが痩せて体験が成立しない | 大 | **高**（D-01 の撤廃で上昇） | 1コミュニティに絞ってローンチ（§2）を**より厳密に**守る。20件未満は件数を出さず空状態コピーで見せる（§6.1） |
...
| **R-10** | **閲覧専用ユーザーが多数派になる**（D-01 でゲーティングを外した直接の帰結） | 大 | 中 | K-8 で監視。40%を超えたら D-01 を再考する（例：未参加時はフィードを当日分だけ遅延表示するなど、**閲覧を止めない範囲での**調整） |
...
| **U-02** | 閲覧専用ユーザーへの働きかけ。D-01 の撤廃により、これは例外ではなく**通常の状態**になった。K-8 を見ながら、圧をかけずに参加へ寄せる方法を決める | M1 |
...
| **参加ゲート** | リアクションを押すために、その日のお題への参加が必要な状態（D-01）。**閲覧は誰でもできる** |
...
| 案 | コピー | 評価 |
| --- | --- | --- |
| **推し** | **Scroll less. Participate more.**（眺めるより、参加しよう。） | プロダクトの主張が最も明確。「見るだけでもいいが、参加した方が面白い」という D-01 の姿勢と一直線に繋がる |
...
| v1.2 | 2026-08-22 | 事前検証 M0 を実施しないと決定（§4）。ゲーティングを撤廃しリアクションを参加者限定に（D-01）。K-2 を 30% に下方修正、K-8・R-10 を新設。技術スタックを確定（U-05 解決）。事業化の意思を確定 |

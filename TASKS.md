...
- [ ] **実機で WebView を確認する** — react-native-webview は web ビルドで
      `does not support this platform` になり、この環境ではコンポーネントを実行できません。
      `make mobile` → `/dev/math` を開いて以下を確認（DECISIONS.md D-029）:

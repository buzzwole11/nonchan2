...
- [x] **`GET /search`** — 保存ライブラリの部分一致検索（タイトル / 著者 / 掲載先 / メモ / 要旨）。
      **コーパスは検索しない**（DECISIONS.md D-041）。`%` `_` はエスケープし、
      順位は SQL の `CASE` で付けてから `LIMIT` する
...
- [x] **Saved の検索ボックス**（250ms デバウンス、どのフィールドで一致したかをチップで表示）。
      検索中は並べ替えチップを隠す（一致フィールド順に並ぶので、効かない操作を出さないため）。
      「検索したが 0 件」と「まだ保存が無い」を別の文言にする
...
### 4-B レンダリング — web では動作確認済み。ネイティブ WebView は実機確認待ち
...
- [x] **タップで全画面、LaTeX コピー、フォント拡大**（`src/math/FormulaSheet.tsx`、
      ...
      コピーは LaTeX 1 つだけ（数式ではコピーと LaTeX コピーが同じ文字列になる — D-042）。
      拡大は倍率で、Dynamic Type の上に乗る。コピーの成否を報告する
- [x] **数式を web でも描画する**（`MathFrame.tsx` / `MathFrame.web.tsx`）— それまで web では
      「does not support this platform」と出ていた。実機が止まっている以上ブラウザが唯一の
      確認手段なので、**Focus Mode の数式は一度も描画されたことが無かった**（DECISIONS.md D-043）
- [x] **はみ出しを測って伝える** — 常時表示のスクロールバー + 文書側の `scrollWidth` 計測 +
      「小さい段階を選ぶと全体が入ります」。横スクロールはジェスチャーなので、
      大きさの段階がその非ジェスチャー代替であることを明示する（仕様書 20 節）
...
- [x] **通知（受信箱まで）**（`services/notification_inbox.py`, migration `0013`,
      `GET /notifications` + 既読, Profile の受信箱）— プリセットと静音時間は**生成時**に
      適用する（クライアントに隠させない：`none` の読者には行自体が無い）。復習期日は
      1 日 1 通に束ね、保存論文の撤回は `important_only` でも届く。判定理由を行に凍結
      （監査）。**push 配信だけが未接続**で、受信箱は同じ行の第 2 の配送先になる
      （DECISIONS.md D-065）
...
- [x] **中間変形候補の生成**（`services/derivation.py`）— 連続する式の間の**リンク**だけを
      生成する。中間式は作らない（CAS が無い以上、捏造になる — D-028）。操作は著者の文の
      引用で、言い換えない。関係ラベルは文の要約であって結論ではない
- [x] **LaTeX → 式の翻訳**（`text/latex_expression.py`）— D-028 の穴を一部塞ぐ。
      読めない構文は**黙って落とさず拒否**する。`\int` を落とす翻訳器は、積分の恒等式を
      算術の恒等式に変えて合格を返す（DECISIONS.md D-046）
- [x] **数値代入による検証**（`MathVerifier` の数値代入部分）— 論文の式は恒等式ではなく
      定義なので、変数を独立に振るチェックは失敗しかしない。前の式を後の式に**代入**する。
      上限は `numerically_spot_checked`（次元も記号操作も走っていない）
- [x] **検証状態の付与と、未検証の既定非表示** — 散文は検証にならない。
      fixture にわざと間違えた行を置き、拒否されることをテストで固定
- [x] **次元解析をパイプラインに接続**（`mathcheck/units.py`）— 記号表の単位を基本次元へ
      変換し、数値検査と同じ式に対して独立に走らせる。**読めない単位は拒否**する
      （「arb. units」「km」など。推測した単位から出した判定は、読者が信用してよいはずの
      `mechanically_verified` を根拠なく引き上げる）。SI 記法と基本次元記法は
      別の関数（`T` はテスラと時間で意味が違い、文字列からは決まらない）
      （DECISIONS.md D-057）
- [x] **人手レビューキュー**（`services/review.py`、`papermatch review`）— triage は
      「必要なカードだけ」を、**理由を言葉で添えて**返す。優先度は理由から導出する
      （別の数値にすると理由と食い違う）。**`human_reviewed` はここからしか到達できない**。
      承認は未検証の変形を昇格させない（査読者は見ていないため）。却下は隠すが消さない。
      決定は CLI のフラグにしない（DECISIONS.md D-047）

---

## Phase 6 — Canvas

- [x] **Mosaic レイアウト**（`services/canvas.py`）— 格子に量子化し、**最も近い空きセル**を
      取る。押しのけないので既存タイルが動かない
- [x] **安定座標** — **t-SNE / UMAP を使わない**。出力が入力集合全体に依存するので、
      論文を 1 本足すと全タイルが動く。射影は**データを一切見ない**（分野 id から決まる
      錨 + 定数 seed の固定基底）ので、安定性が構造的（DECISIONS.md D-048）。
      実データで確認: 12 枚配置してセル重複 0、1 本追加して動いたタイル 0 枚
- [x] **分野の知覚的グラデーション**（`packages/design-tokens/src/fieldColor.ts`）—
      Oklab で混色。sRGB のチャンネル平均は**中点より暗くなる**（実測 0.51 vs 0.59）。
      13 節は明るさを「最近の活動」に使うので、暗くなる混色は別の変数について
      誤った値を報告するタイルになる
- [x] **選択時の浮遊と周辺の強調**（`src/canvas/CanvasPlane.tsx`）— 1.15 倍に浮き、近くが
      少し離れ、他が暗くなる。**下の座標は動かない**ので、選択を解くと元に戻る。
      Reduce Motion では移動だけを外す（枠・明度・パネルが選択を伝える）
- [x] **フィルターとクラスタズーム** — 島をタップすると絞り込んで 1 段階寄る。
      絞り込みは隠すだけで、残ったタイルは動かない
- [x] **ズーム段階で描くものを変える**（`GET /canvas`、「眺める」タブ）— 遠景は
      **分野島と件数**（全体を画面に収めると 1 セルが約 5pt になり、タイルは描けない）、
      近景はタイルで 2 軸パン（DECISIONS.md D-049）
- [x] **手動配置**（`PATCH /canvas/{paperId}`）— 方向ボタン。ドラッグはスイッチや
      キーボードの利用者が唯一まったく実行できない操作なので、ボタンを主にする
- [x] **Library との切替**（`saved.tsx` の表示チップ、`src/canvas/CanvasView.tsx`）—
      14 節は Library と Canvas を**同じデータの 2 つの表示**として書いているので、
      別タブをやめて Saved 内の切替にした。**選択は画面が持ち**、表示を切り替えても
      同じ論文が選ばれたまま（14 節「逆方向も位置関係を失わない」）
- [x] **連続変形のアニメーション**（`src/canvas/morph.ts`, `MorphTargets.tsx`,
      `MorphLayer.tsx`）— 選択中の論文が Library の行と Canvas 上の位置の間を移動する。
      両端の**矩形・角丸・色は描いた側が登録する**（矩形から推測しない）。遠景では
      タイルが無いので**分野島**が着地点になる。片方が画面外・大きさ 0 のときは
      変形せず単純に切り替える。Reduce Motion は即時切替（20 節が許している）
      （DECISIONS.md D-052）
...
- [x] **共有画像の書き出し**（`src/share/shareCard.ts`, `src/share/ShareSheet.tsx`）—
      **私的な項目（自分のメモ・保存理由）は既定で off、それ以外は既定で on**。
      ...
      （黙っていると「メモを書いていない」と区別がつかない）。原文リンクと
      ライセンスは選択不可で常に入る（21 節）。SVG なのでテストで中身を検証できる
      （DECISIONS.md D-051）
...
- [ ] Reduce Motion を有効にして、連続変形が即時切替になること
